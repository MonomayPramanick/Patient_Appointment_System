...
public class PatientRegister extends JFrame implements ActionListener {
    ...
    PatientRegister() {
        ...
        l11 = new JLabel("Problem Faced:");
        l11.setBounds(100, 430, 200, 30);
        l.add(l11);

        t6 = new JTextArea();
        t6.setBounds(250, 430, 200, 60); 
        t6.setLineWrap(true);
        l.add(t6);

        l12 = new JLabel("Doctor name:");
        l12.setBounds(100, 500, 200, 30); 
        l.add(l12);

        t11 = new JTextField();
        t11.setBounds(250, 500, 200, 30); 
        l.add(t11);
   ...
   public void actionPerformed(ActionEvent e) {
    if (e.getSource() == b1) {
        ...
        String problemFaced = t6.getText();
        String doctor = t11.getText();
        ...
        try {
            ...
            String idCheckQuery = "SELECT * FROM patient WHERE patient_id = '" + id + "'";
            ResultSet rs = obj.stm.executeQuery(idCheckQuery);
            if (rs.next()) {
                JOptionPane.showMessageDialog(f, "Patient ID already exists. Registration failed.", "Error", JOptionPane.ERROR_MESSAGE);
                return; // Exit the method
            }

            // Perform database operations to register the patient
            String query = "INSERT INTO patient (name, patient_id, username, city, email, gender, phone, age, password, problem, doctor_name, date) "
                    + "VALUES ('" + name + "', '" + id + "', '" + username + "', '" + city + "', '" + email + "', '" + gender + "', '" + phone + "', '" + age + "', '" + password + "', '" + problemFaced + "', '" + doctor + "', '" + date + "')";
